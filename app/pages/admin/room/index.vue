...
<template>
  <div class="min-h-screen flex flex-col bg-[#F5F2EB] text-[#2A2825] selection:bg-[#FBDA0C] selection:text-[#2A2825]">
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 lg:px-8 py-8 space-y-8">
      ...
      <div v-if="successMessage" class="p-4 rounded-2xl bg-[#FFF8D6] border border-[#FBDA0C] text-xs text-[#6B5900] flex items-center justify-between shadow-sm animate-fade-in">
        ...
        <button @click="successMessage = ''" class="text-[#6B5900] hover:text-[#2A2825] text-sm font-bold">&times;</button>
      </div>

      <!-- Error Notification Banner -->
      <div v-if="errorMessage" class="p-4 rounded-2xl bg-[#F5E6E3] border border-[#E5C7C2] text-xs text-[#8C4E43] flex items-center justify-between shadow-sm">
        <div class="flex items-center gap-2.5">
        ...
        <button @click="errorMessage = ''" class="text-[#8C4E43] hover:text-[#2A2825] text-sm font-bold">&times;</button>
      </div>


      <!-- SECTION 1: BUILDINGS MANAGEMENT -->
      <section class="space-y-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2 border-b border-[#E2DACF]">
          <div>
            <h2 class="text-lg font-bold text-[#2A2825] flex items-center gap-2">
              Buildings
              <span class="text-xs px-2 py-0.5 rounded-full bg-[#EFEBE4] text-[#736D66] font-semibold border border-[#E2DACF]">
                {{ buildings.length }}
              </span>
            </h2>
        ...
        <div v-else-if="buildings.length === 0" class="japandi-card rounded-2xl p-8 text-center bg-[#FAF8F5] border border-[#E2DACF]">
          ...
          >
            <div>
              <div class="flex items-start justify-between gap-2 mb-2">
                ...
                <span class="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-[#FFF5C2] text-[#6B5900] border border-[#E6C200] shrink-0">
            ...
            <div class="mt-4 pt-3 border-t border-[#E2DACF] flex items-center justify-between text-xs">
              <button
                @click="openCreateRoomModal(bld.id)"
              ...
              <div class="flex items-center gap-2">
                <button
                  @click="openEditBuildingModal(bld)"
                ...
                <button
                  @click="promptDelete('building', bld.id, bld.name)"
      ...
      <section class="space-y-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-[#E2DACF]">
          <div>
            ...
            <p class="text-xs text-[#736D66]">Manage individual room mailboxes, building assignments, and display names.</p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="relative min-w-[200px]">
              <input
                v-model="searchQuery"
                type="text"
                placeholder="Search room name or email..."
              ...
              />
              <svg class="w-4 h-4 text-[#736D66] absolute left-3 top-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div v-else-if="filteredRooms.length === 0" class="japandi-card rounded-2xl p-10 text-center bg-[#FAF8F5] border border-[#E2DACF]">
          <svg class="w-10 h-10 text-[#736D66] mx-auto mb-3 opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <p class="text-xs font-semibold text-[#2A2825]">No rooms match your current criteria.</p>
          <p class="text-xs text-[#736D66] mt-1">Try adjusting your search query or building filter, or click "Add Room" to create a new entry.</p>
        </div>

        <div v-else class="overflow-x-auto rounded-2xl border border-[#E2DACF] bg-[#FAF8F5] shadow-sm">
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-[#EFEBE4] border-b border-[#E2DACF] text-[11px] uppercase tracking-wider text-[#736D66] font-extrabold">
            ...
            <tbody class="divide-y divide-[#E2DACF] text-xs">
              <tr 
                ...
                :key="rm.id"
                class="hover:bg-[#FFF5C2]/40 transition-colors"
              >
                <!-- Room Name -->
                ...
                <td class="py-3.5 px-4">
                  <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#EFEBE4] text-[#2A2825] font-semibold text-[11px] border border-[#E2DACF]">
                ...
                <td class="py-3.5 px-4 text-right">
                  <div class="flex items-center justify-end gap-2">
                    <button
                      @click="openEditRoomModal(rm)"
                      class="px-2.5 py-1 rounded-lg bg-[#EFEBE4] hover:bg-[#E2DACF] text-[#2A2825] text-[11px] font-semibold border border-[#E2DACF] transition-colors"
                    >
                      Edit
                    </button>
                    <button
                      @click="promptDelete('room', rm.id, rm.name)"
                      class="px-2.5 py-1 rounded-lg bg-[#F5E6E3] hover:bg-[#EAD7D4] text-[#8C4E43] text-[11px] font-semibold border border-[#E5C7C2] transition-colors"
                    >
                      Delete
                    </button>
    ...
    <div v-if="isBuildingModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
      <div class="japandi-card w-full max-w-md rounded-3xl p-6 bg-[#FAF8F5] border border-[#E2DACF] shadow-xl space-y-5 animate-scale-up">
        <div class="flex items-center justify-between pb-3 border-b border-[#E2DACF]">
          ...
          <button @click="isBuildingModalOpen = false" class="text-[#736D66] hover:text-[#2A2825] text-lg font-bold">&times;</button>
        </div>

        <div v-if="buildingFormError" class="p-3 rounded-xl bg-[#F5E6E3] border border-[#E5C7C2] text-xs text-[#8C4E43]">
          {{ buildingFormError }}
        </div>
        ...
        <form @submit.prevent="handleSaveBuilding" class="space-y-4 text-xs">
          ...
          <div>
            <label class="block font-bold text-[#2A2825] mb-1">Description <span class="text-[#736D66] font-normal">(Optional)</span></label>
            <textarea
          ...
          <div class="flex items-center justify-end gap-3 pt-3 border-t border-[#E2DACF]">
            <button
              type="button"
              @click="isBuildingModalOpen = false"
    ...
    <div v-if="isRoomModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
      <div class="japandi-card w-full max-w-lg rounded-3xl p-6 bg-[#FAF8F5] border border-[#E2DACF] shadow-xl space-y-5 animate-scale-up">
        <div class="flex items-center justify-between pb-3 border-b border-[#E2DACF]">
          <h3 class="text-base font-extrabold text-[#2A2825]">
            {{ editingRoom ? 'Edit Room' : 'Add New Room' }}
          </h3>
          <button @click="isRoomModalOpen = false" class="text-[#736D66] hover:text-[#2A2825] text-lg font-bold">&times;</button>
        </div>
        ...
        <form @submit.prevent="handleSaveRoom" class="space-y-4 text-xs">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                v-model="roomForm.email"
                type="email"
                placeholder="e.g. conf3a@company.com"
                class="w-full px-3.5 py-2.5 rounded-xl bg-[#EFEBE4] border border-[#E2DACF] text-xs text-[#2A2825] placeholder-[#736D66] focus:outline-none focus:border-[#FBDA0C]"
                required
              />
            </div>
    ...
    <div v-if="isDeleteModalOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
      <div class="japandi-card w-full max-w-sm rounded-3xl p-6 bg-[#FAF8F5] border border-[#E2DACF] shadow-xl text-center space-y-4 animate-scale-up">
        <div class="w-12 h-12 mx-auto rounded-2xl bg-[#F5E6E3] border border-[#E5C7C2] text-[#8C4E43] flex items-center justify-center">
        ...
        <div>
          ...
          <p class="text-xs text-[#736D66] mt-1.5 leading-relaxed">
            Are you sure you want to delete <strong class="text-[#2A2825] font-bold">"{{ deleteTarget?.name }}"</strong>? This action cannot be undone.
          </p>
        </div>
